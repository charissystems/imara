...
import { getTenantDb } from '../config';
 ...
 */
accountingRoutes.post('/accounts', validate(createAccountSchema), async (c) => {
    try {
        ...
        if (!user || !hasPermission(user.role || '', 'chart_of_accounts', 'create')) {
 ...
 */
accountingRoutes.patch('/accounts/:id', validate(updateAccountSchema), async (c) => {
    try {
        const { id } = c.req.param();
        ...
        if (!user || !hasPermission(user.role || '', 'chart_of_accounts', 'update')) {
            throw new UnauthorizedError('Insufficient permissions');
        }
 ...
 */
accountingRoutes.post('/periods', validate(createFinancialPeriodSchema), async (c) => {
    try {
 ...
 */
accountingRoutes.patch('/periods/:id/close', async (c) => {
    try {
        const { id } = c.req.param();
        const { schema_name } = c.get('tenant')!;
        const user = c.get('user');
        ...
        if (!user || !hasPermission(user.role || '', 'chart_of_accounts', 'approve')) {
            throw new UnauthorizedError('Insufficient permissions to close financial period');
        }
 ...
 */
accountingRoutes.post('/year-end-close', async (c) => {
    try {
        const { schema_name } = c.get('tenant')!;
        const user = c.get('user');
        const db = getTenantDb(schema_name);
        ...
        if (!user || !hasPermission(user.role || '', 'chart_of_accounts', 'approve')) {
            throw new UnauthorizedError('Insufficient permissions for year-end closing');
        }
 ...
 */
accountingRoutes.post('/journals', validate(createJournalEntrySchema), async (c) => {
    try {
 ...
 */
accountingRoutes.post('/journals/:id/submit', async (c) => {
    try {
        const { id } = c.req.param();
        const { schema_name } = c.get('tenant')!;
        const user = c.get('user');
 ...
 */
accountingRoutes.post('/journals/:id/approve', validate(journalActionSchema), async (c) => {
    try {
        const { id } = c.req.param();
        const { schema_name } = c.get('tenant')!;
        const user = c.get('user');
        ...
        if (!user || !hasPermission(user.role || '', 'journal_entries', 'approve')) {
            throw new UnauthorizedError('Insufficient permissions to approve journal entries');
        }
 ...
 */
accountingRoutes.post('/journals/:id/reject', validate(journalActionSchema), async (c) => {
    try {
        const { id } = c.req.param();
        const { schema_name } = c.get('tenant')!;
        const user = c.get('user');
        ...
        if (!user || !hasPermission(user.role || '', 'journal_entries', 'approve')) {
            throw new UnauthorizedError('Insufficient permissions to reject journal entries');
        }
 ...
 */
accountingRoutes.post('/journals/:id/post', async (c) => {
    try {
        const { id } = c.req.param();
        const { schema_name } = c.get('tenant')!;
        const user = c.get('user');
        ...
        if (!user || !hasPermission(user.role || '', 'journal_entries', 'approve')) {
            throw new UnauthorizedError('Insufficient permissions to post journal entries');
        }
